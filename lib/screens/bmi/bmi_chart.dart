...
class _BMIDataChartState extends State<BMIDataChart> {
  ...
  Future<void> _fetchBMIData() async {
    try {
      ...
      final weekAgo = now.subtract(const Duration(days: 10)); // 7 days ago
    ...
    latestBMIs.keys.forEach((date) {
      ...
      final List<DataPoint<DateTime>> newDataPoints = latestBMIs.entries.map((entry) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('BMI Chart', style: TextStyle(color: Color(0xFF40D876))),
        centerTitle: true,
      ...
      body: Padding(
        ...
        child: Container(
          ...
          child: LineChart(
            LineChartData(
              minX: dataPoints.isNotEmpty ? dataPoints.first.x.millisecondsSinceEpoch.toDouble() : 0,
              maxX: dataPoints.isNotEmpty ? dataPoints.last.x.millisecondsSinceEpoch.toDouble() + 86400000.0 : 1,
              ...
              borderData: FlBorderData(
                ...
                border: const Border.symmetric(
                  horizontal: BorderSide(color:  Color(0xff68737d)), // Set color to match x-axis text color
                  vertical: BorderSide(color:  Color(0xff68737d)), // Set color to match y-axis text color
                ),
              ),
              lineBarsData: [
                LineChartBarData(
                  spots: dataPoints.map((dataPoint) {
                    return FlSpot(dataPoint.x.millisecondsSinceEpoch.toDouble(), dataPoint.y);
                  }).toList(),
                  isCurved: true,
                  colors: [Colors.blue],
                  barWidth: 2,
  ...
  double _calculateMinBMI() {
    return dataPoints.map((e) => e.y).reduce((value, element) => value < element ? value : element);
  }
